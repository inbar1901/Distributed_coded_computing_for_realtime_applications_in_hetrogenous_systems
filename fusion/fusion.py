...
def main():
    ...
    producer1_queue_name = "working_producer_1"
    producer1_channel = channel.queue_declare(queue=producer1_queue_name)  # message queue = sends work to workers
    producer2_queue_name = "working_producer_2"
    producer2_channel = channel.queue_declare(queue=producer2_queue_name)  # message queue = sends work to workers
    ...
    channel.basic_consume(queue=producer1_queue_name, on_message_callback=callback)
    channel.basic_consume(queue=producer2_queue_name, on_message_callback=callback)
